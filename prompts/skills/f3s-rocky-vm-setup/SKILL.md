...
- [Dotfiles deployment (gonf)](references/gonf.md) — `~/git/dotfiles/gonf.sh home` (paul), rocky tmux overrides loaded by `tmux.conf` itself (`home_tmux_rocky` is a legacy alias of `home_tmux`); no Rocky package task (install packages with `dnf` as root)
