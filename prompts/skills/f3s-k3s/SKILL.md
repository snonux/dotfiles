...
- [r-node Deploy (gonf)](references/r-node-deploy.md) — reusable gonf rollout to r0/r1/r2 (`./gonf.sh cluster rocky-k3s rnodes_nfs_mount_monitor`, recipe `gonf/rnodes/maintenance.go`): root SSH, parallel 3, idempotent `InstallFile` + change-gated daemon-reload/timer restart
