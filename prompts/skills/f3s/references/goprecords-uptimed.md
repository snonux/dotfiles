...
1. On the Mac, **`.uprecords-sync.sh collect`** copies the local **uptimed** records into the **worktime** git repo as
...
2. On **earth** (the only host that publishes), **`.uprecords-sync.sh import`** reads those repo files and **`PUT`**s them to goprecords, **re-labelling** the raw host `MBDVXJ4XKH9C` → **`mega-m3-pro`**:
...
3. Automation: `goprecords-upload-earth.service` has a **second `ExecStart`** that runs the import hourly alongside earth's own upload:
   ...
   ExecStart=/bin/sh %h/git/worktime/.uprecords-sync.sh import
   ```
